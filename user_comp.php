        		...
        		<? foreach($donnees as $d): ?>
					...
					<?endif; //endGestion des catégories ?>
								<tr>
									<td><?= $d->designation ?></td>
									<td><? for($i = 1; $i<6; $i++): ?>
											<input type="radio" name="<?= $d->designation ?>" value="<?= $i ?>" class="star" disabled="true" <? if($i == $_POST[$d->designation]): ?>checked="checked"<? endif; ?> />
										<? endfor; ?>
									</td>
									<td><input type="text" name="<?= $d->designation.'.commentaire' ?>" disabled="true" <? if($_POST[$d->designation.'_commentaire']): ?>value="<?= $_POST[$d->designation.'_commentaire'] ?>"<? endif; ?> /></td>
								</tr>
					<? $cat = $d->categorie; ?>
				<? endforeach; ?>
							</table>
